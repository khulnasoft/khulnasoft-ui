...
const CellGroup = ({ cell, row, header, testPrefix, coloredSortedColumn }) => {
  ...
  return (
    <Flex
      ...
      position="relative"
      data-testid={`khulnasoft-table-cell-${cell.column.columnDef.id}${testPrefix}`}
      overflow="hidden"
...
export default memo(
  ({
    ...
    return (
      <>
        <Flex
          data-testid={`khulnasoft-table-row${testPrefix}${
        ...
        >
          ...
          {renderSubComponent && row.getIsExpanded() && !row.getIsGrouped() && (
            <Flex
              flex
              data-testid={`khulnasoft-table-sub-row${testPrefix}${
                testPrefixCallback ? "-" + testPrefixCallback(row.original) : ""
              }`}
